...
- [文档] 新增全市场盘后选股推荐规格，明确股票池、规则引擎、CSV 快照、调度、回测和当前代码边界。
- [新功能] 新增全市场盘后选股推荐后端 MVP 与 Web 历史页，支持默认 TOML profile、全市场行情快照、规则筛选评分、CSV/meta 落盘、API 查询/下载、DB 摘要索引、可选 LLM 复核、通知摘要、文件型回测、有效交易日落盘和独立盘后调度入口。
- [修复] 个股推荐快照下载新增腾讯批量行情与 AkShare 新浪全市场 fallback，并为推荐评分历史 K 线加入腾讯快速路径，避免东方财富链路不可用时生成空结果或运行过慢。
### 发布亮点
