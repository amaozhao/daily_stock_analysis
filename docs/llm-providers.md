...
| MiniMax Token Plan | `minimax` | `anthropic` | `https://api.minimaxi.com/anthropic` | `MiniMax-M2.7,MiniMax-M2.7-highspeed` |
| MiniMax OpenAI-compatible | `minimax_paygo`（示例） | `openai` | `https://api.minimax.io/v1` | `MiniMax-M2.7,MiniMax-M2.7-highspeed` |
...
| MiniMax Token Plan | [Token Plan 快速接入](https://platform.minimaxi.com/docs/token-plan/quickstart)、[Anthropic API 兼容](https://platform.minimaxi.com/docs/api-reference/text-anthropic-api)、[获取模型列表](https://platform.minimaxi.com/docs/api-reference/models/anthropic/list-models) | Token Plan Key 与按量 API Key 不互通；订阅套餐推荐使用 Anthropic 兼容 Base URL `https://api.minimaxi.com/anthropic`，运行时模型写成 `anthropic/MiniMax-M2.7`。极速套餐可改用 `MiniMax-M2.7-highspeed`。 |
| MiniMax OpenAI-compatible | [OpenAI API 兼容](https://platform.minimax.io/docs/api-reference/text-chat)、[获取模型列表](https://platform.minimax.io/docs/api-reference/models/openai/list-models) | 官方 OpenAI-compatible Base URL 为 `https://api.minimax.io/v1`，并列出 `MiniMax-M2.7`、`MiniMax-M2.7-highspeed`；如需同时保留 Token Plan，可用自定义渠道名（例如 `minimax_paygo`）避免覆盖默认 `minimax` 预设。中国区 Coding 工具场景可能使用 `.com`/Anthropic 专用入口，以控制台为准。 |
